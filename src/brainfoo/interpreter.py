...
from brainfoo.errors import *


class BrainfkInterpreter:
    """Brainfk interpreter/debugger"""
    def __init__(self, file: str | Path | None = None, code: str | None = None, stdin=None, tick_callback=None):
        """Initialize self

        stdin: optional reference to an input widget (e.g., CharInput) providing bytes via .bytes list
        tick_callback: optional callable(cell_index:int, value:int) called after each instruction
        """
        self.cell_index = 0
    ...
    def run(self) -> None:
        ...
        while self.instruction_index < len(self.code):
            ...
            match instruction:
                ...
                case "]":
                    if self.cells[self.cell_index] != 0:
                        self.instruction_index = self.find_bracket_match(self.instruction_index, False)
